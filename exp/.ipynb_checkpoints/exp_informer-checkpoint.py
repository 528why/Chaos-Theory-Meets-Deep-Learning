...
#                 print(name, param.data)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            #     np.savetxt(f, train_loss, delimiter=",")
            ...
            # 构建包含时间的文件名
            filename = f"train_loss_{current_time}.txt"
            # 打开文件并写入数据
...
def chaos(n):
    ...
    list_chaos = df[0]
    list_chaos = list_chaos[1:n+1]
    np_chaos = np.array(list_chaos)
    return np_chaos

# def chaos(n):
#     np_chaos = np.load('lorenz2560.npy')
